...
function createDailyReportForm() {
  ...
  // 1. 作業日
  form.addDateItem().setTitle("■作業日").setRequired(true);
  ...
  form.addCheckboxItem()
    .setTitle("■作業員")
    .setChoiceValues(["新妻", "上村", "その他職員", "高岡", "前澤", "齋藤", "藤田", "菅生", "田谷", "その他作業員"])
    .showOtherOption(true)
    .setRequired(true);
  ...
  form.addListItem().setTitle("■波向").setChoiceValues(directions);
  form.addTextItem().setTitle("■波高").setHelpText("数値のみ入力（単位：m）");
  form.addListItem().setTitle("■風向").setChoiceValues(directions);
  form.addTextItem().setTitle("■風速").setHelpText("数値のみ入力（単位：m/s）");
